...
The Bundesliga reference plugin is a WASM component implementing `fulltime-plugin-api`'s
`data-provider` interface, wrapping `openligadb`'s existing fetch logic rather than
reimplementing HTTP calls against the OpenLigaDB API. It is the first plugin the FullTime
desktop app's plugin host runtime loads, and serves as the reference implementation proving
the `fulltime-plugin-api` contract is sufficient before any second plugin is attempted. This
capability governs the plugin's data-provider implementation, its network access model, its
schema mapping, and its manifest declaration.
